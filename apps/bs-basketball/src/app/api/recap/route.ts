...
// Shared `ai_cache` table with BS Football — every key is prefixed `bball:`
// so md5 collisions across sports can't surface the wrong-sport recap.
...
export async function POST(request: Request) {
  try {
    ...
    const { games, season, dayIndex, isPlayoffs, playoffRound } = await request.json();
    if (!games || !Array.isArray(games) || games.length === 0) {
      return NextResponse.json({ error: 'games array required' }, { status: 400 });
    }

    // Check persistent cache (Supabase → in-memory fallback)
    const key = cacheKey({ games, season, dayIndex });
    const cached = await getCache(key);
    if (cached) {
      return NextResponse.json({ topics: cached });
    }

    // Basketball round naming — bracket.ts uses 1 = First Round, 2 = Conf Semis,
    // 3 = Conf Finals, 4 = Finals. Use the actual labels the bracket emits.
    const dayContext = isPlayoffs
      ? `This is the PLAYOFFS — ${
          playoffRound === 1 ? 'First Round'
          : playoffRound === 2 ? 'Conference Semis'
          : playoffRound === 3 ? 'Conference Finals'
          : playoffRound === 4 ? 'the NBA Finals'
          : `Playoff Round ${playoffRound ?? '?'}`
        }. The stakes are EVERYTHING.`
      : `This is day ${dayIndex ?? '?'} of an 82-game regular season.`;

    // System prompt is static → use prompt caching to reduce input token costs ~90%
    const systemPrompt = `You write "Hoops Tonight" — a post-game breakdown. Two commentators do a deep dive on the user's team game.

COMMENTATORS:
- **Mike Hartwell** (speakerId: "stats") — Analytics guy. Dry wit, historical parallels. Uses actual stats — points, true shooting, pace, plus-minus.
- **Trey "Big Shot" Daniels** (speakerId: "hottake") — Passion guy. CAPS, bold declarations, vivid metaphors.

Generate 3-4 topics about THIS GAME:
1. **Game result** — headline = score (e.g. "Timberwolves 122, Mavericks 109"). Who won and why.
2. **Lead scorers' matchup** — compare both teams' leading scorers. e.g. "Doncic's 38-7-8 vs Edwards' 42-5-3". Who got the better of it?
3. **Key turning point** — what decided it? A third-quarter run, a big-time bucket, a defensive stop, a foul-out, the bench unit?
4. **What's next** — what this means for both teams going forward (seeding, rotation, momentum).

Each topic: 3 exchanges. Context field = box-score summary (final score, leading scorers, rebounds/assists leaders).
...
Return ONLY the JSON array.`;
    ...
    const userContent = `Season ${season}, ${dayContext}\n\nGAME:\n${JSON.stringify(games)}`;
    let raw = '';
...
- Mike should cite specific stats from the game data and break down what they mean.
- Trey should be dramatic, use CAPS for emphasis, and react with real emotion to big runs, dunks, and clutch shots.
- Each topic MUST have at least 3-4 exchanges showing real back-and-forth debate between Mike and Trey — they should RESPOND to each other, agree, disagree, and build on each other's points.
- The "context" field should include a detailed box-score summary with key stats (leading scorers, rebound and assist leaders, shooting splits).
- DO NOT be brief. The user is reading this for entertainment — make it feel like a real post-game show with personality, banter, and analysis.` },
    ...
    try {
      ...
      console.error('Recap API JSON parse failed. Raw (first 500):', raw.slice(0, 500), parseErr instanceof Error ? parseErr.message : parseErr);
