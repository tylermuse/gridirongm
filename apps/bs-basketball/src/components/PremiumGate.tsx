...
export function PremiumGate({
  ...
  const copy = feature
    ...
    : { title: 'Upgrade to Premium', description: 'Unlock the full BS Hoops experience for $4.99/mo — one sub also unlocks BS Football.' };
    ...
    return (
      <div className={`relative ${className ?? ''}`}>
        ...
        <div className="absolute inset-0 flex items-center justify-center p-4">
          <div
            className="bg-white border rounded-xl shadow-lg p-4 max-w-sm text-center"
          ...
          >
            ...
            <Link
              href={upgradeHref}
              className="inline-block mt-3 px-4 py-1.5 rounded-lg text-white text-xs font-bold hover:brightness-110 transition"
  ...
  return (
    <div
      className={`rounded-xl border-2 border-dashed p-6 text-center ${className ?? ''}`}
      style={{
    ...
    >
      ...
      <Link
        href={upgradeHref}
        className="inline-block mt-4 px-5 py-2 rounded-lg text-white text-sm font-bold hover:brightness-110 transition"
        style={{ background: 'var(--accent)' }}
      >
        Upgrade to Premium
      </Link>
