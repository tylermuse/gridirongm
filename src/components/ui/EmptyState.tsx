...
export function EmptyState({ icon, title, description, cta, ctaHref }: {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 px-8 text-center max-w-md mx-auto">
      <div className="w-20 h-20 rounded-2xl bg-[var(--surface-2)] flex items-center justify-center mb-6 text-4xl">{icon}</div>
      ...
      {cta && ctaHref && (
        <Link href={ctaHref} className="bg-blue-600 text-white px-6 py-2.5 rounded-lg font-medium hover:bg-blue-700 transition">{cta}</Link>
      )}
    </div>
  );
}
